...
import DraggableTree, { PropDraggable, BeforeDragOpen, context } from '../components/DraggableTree';
...
const beforeDragOpen: BeforeDragOpen = (stat) => {
	if (stat.data.text === 'Animals' && stat.children.length === 1) {
		context.targetTree!.addMulti([{ text: 'add 1' }, { text: 'add 2' }, { text: 'add 3' }], stat);
	}
};
