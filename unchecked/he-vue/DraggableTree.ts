...
const cpt = defineComponent({
	...
	props: {
		...
		resolveStartMovePoint: {
			type: [String, Function] as PropType<'mouse' | 'default' | ((dragElement: HTMLElement) => Point)>
		},
		/**
			...
			targetTree!.ignoreUpdate(() => {
				...
				if (!targetTree!.has(targetTree!.placeholderData)) {
					if (targetTree!.table) {
						...
						if (tr) {
							for (const { value: childEl } of hp.iterateAll<HTMLTableCellElement>(tr.children)) {
		...
		this.treeDraggableInstance = extendedDND(rootEl, {
			...
			onDragStart: (event) => {
				...
				}
				{
					...
					event.dataTransfer?.setDragImage(dragElement, clientX - x, clientY - y);
				}
				...
				targetTree = this;
				event.dataTransfer?.setData('text', `he-tree drag start at ${new Date().toISOString()}`); // required for Chrome Andriod, or Drag and Drop API does't  work
			...
			onDragOver: hp.applyFinally(
				(event: DragEvent) => {
					...
					nodeList.forEach((el) => {
						if (!hp.hasClassIn(el, ['drag-placeholder-wrapper', 'dragging-node']) && hp.css(el, 'display') !== 'none') {
							nodeEls.push(el as HTMLElement);
						}
					});
					const t = hp.binarySearch(
						nodeEls,
						(node) => hp.getBoundingClientRect(node)[!btt ? 'top' : 'bottom'] - movePoint.y,
					...
					const prevX_minusMovePointX = hp.cacheFunction(() =>
						!rtl ? prevBCR().x - movePoint.x : movePoint.x - (prevBCR().x + prevBCR().width)
					).action;
					const atPrevIndentRight = hp.cacheFunction(() =>
						!rtl ? movePoint.x > prevBCR().x + indent : movePoint.x < prevBCR().x + prevBCR().width - indent
					).action;
					//
			...
			onDrop: (event) => {
				...
				let dragChanged = (() => {
					...
					if (!targetTree!.has(targetTree!.placeholderData)) {
						...
						const placeholder = targetTree!.getStat(targetTree!.placeholderData);
						if (startTree === targetTree && placeholder.parent === dragNode!.parent) {
							if ((hp.findTreeData(dragNode, (node) => node === placeholder), { childrenKey: CHILDREN })) {
							} else {
