...
const cpt = defineComponent({
	...
	setup(props) {
		...
		const end2 = computed(() => hp.notGreaterThan(end.value, (props.items?.length || 1) - 1));
		const avgSize = ref(0);
		const startSize = computed(() => (positions.value[start.value] ? getPosition(start.value) : 0));
		const totalSize = computed(() =>
			positions.value.length > 0 ? getPosition(positions.value.length - 1) + hp.arrayLast(sizes.value) : 0
		);
		const endSize = computed(() =>
			positions.value[end2.value] ? totalSize.value - getPosition(end2.value) - sizes.value[end2.value] : 0
		);
		const listStyle = computed(() => (!props.disabled ? { overflow: 'auto' } : {}));
		...
		async function update() {
			...
			function getStart() {
				...
				const r = hp.binarySearch(positions.value, (mid) => mid - startPosition, {
			...
			function getEnd() {
				...
				const r = hp.binarySearch(positions.value, (mid) => mid - endPosition, {
					returnNearestIfNoHit: true
				})!;
				return r.index;
			}
