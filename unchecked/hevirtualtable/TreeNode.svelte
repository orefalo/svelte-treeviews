<script lang="ts">
	...
	function computeHLineStyle(rtl: boolean, indent: number, stat: Stat<any>, treeLineOffset: number): string {
		const pos = (stat.level - 2) * indent + treeLineOffset;
		const leftOrRight = rtl ? 'right' : 'left';
		return leftOrRight + ':' + pos + 'px';
	}

	function computeInlineStyle(rtl: boolean, indent: number, stat: Stat<any>): string {
		return (!rtl ? 'paddingLeft:' : 'paddingRight:') + indent * (stat.level - 1) + 'px';
	}

	function hasNextVisibleNode(node: Stat<any>) {
	...
	$: vLines = (() => {
		...
		let current: Stat<any> | null = stat;
