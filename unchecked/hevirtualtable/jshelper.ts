...
export function stylesDictToString(styles: object) {
	// @ts-ignore
 ...
 * @returns
 */
export function empty(v: null | undefined | boolean | number | object | unknown[]) {
	if (v == null) {
		return true;
 ...
 * @returns
 */
export function getArrayItemSiblingsByOffsets<T>(arr: T[], item: any, offsets: number[]) {
 ...
 * @returns
 */
export function arrayDistinct<T>(arr: T[], getMark?: (item: T, index: number) => unknown) {
	const t = new Set();
 ...
 * @returns
 */
export function arrayFilter<T extends ArrayLike<any>>(arr: T, without: any[]): T {
	// @ts-ignore
 ...
 * @returns
 */
export function dotGet(obj: object, path: string | string[], throwError?: boolean): any {
 ...
 * @param sourceKey
 */
export function objectDefineProxyProperty(targetObj: object, targetKey: string, sourceObj: object, sourceKey: string) {
 ...
 * @returns
 */
export function mapObjects<T>(arr: T[], idKey: string | ((item: T, index: number) => string)): { [key: string]: T } {
	const r = {};
 ...
 * @param opt
 */
export function walkTreeData<T>(obj: T | T[], handler: WalkTreeDataHandler<T>, opt: WalkTreeDataOptions = {}) {
...
export function cloneTreeData<T>(root: T, options?: { childrenKey?: string; nodeHandler?: CloneTreeNodeHandler<any> }) {
 ...
 * @returns
 */
export function watchChange<T>(getVal: ReplaceReturnType<OmitFirstParameter<T>, any>, handler: T) {
	let oldVal;
	const update = (...args) => {
		// @ts-ignore
 ...
 * @returns
 */
export function applyFinally<T extends Function, Y extends Function>(func: T, finallyFunc: Y) {
 ...
 */
export function continuous<T extends (info: { count: number; currentCount: { value: number } }, ...args: any[]) => any>(
	method: T,
	opt?: { strategy: 'every' | 'last'; resetCount?: boolean }
) {
	// default options
	if (!opt) {
		// @ts-ignore
 ...
 */
export function viewportPositionToFixed(el: Node, viewportPosition: { x: number; y: number }) {
 ...
 * @returns
 */
export function getScrollSpace(el: Element, direction: 'left' | 'right' | 'top' | 'bottom') {
	let space: number;
	if (direction === 'left') {
		space = el.scrollLeft;
...
export function openPreviewWindow(previewUrl: string, name = 'preview', width = 1000, height = 800) {
...
// for animation
export function easeInOutQuad(startValue: number, changeInValue: number, changedTime: number, duration: number) {
	let t = changedTime,
		d = duration,
		b = startValue,
		c = changeInValue;
...
// typescript
export type ParametersWithoutFirst<T extends (...args: any) => any> = T extends (first: any, ...args: infer P) => any
	? P
	: never;
