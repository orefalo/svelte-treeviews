<script lang="ts">
	...
	export let getParentId = (x) => getParentNodePath(x[propNames.nodePathProperty]);
	...
	$: canNest =
		(highlightedNode?.[propNames?.insertDisabledProperty] || canNestPos || canNestTime) &&
	...
	$: parentChildrenTree = OrderByPriority(
		getParentChildrenTree(filteredTree ? filteredTree : tree, parentId, isChild, getParentId),
		propNames
	);
	...
	function toggleExpansion(node, expanded) {
		...
		if (val && expandCallback != null && node[propNames.useCallbackProperty] == true) {
	...
	function ComputeVisualTree(filteredTree) {
		tree = computeInitialVisualStates(tree, isChild, getParentId, filteredTree ?? tree, propNames);
	}
	...
	function handleDragDrop(e, node, el) {
		...
		console.log(draggedPath + ' dropped on: ' + node[propNames.nodePathProperty]);
		...
		let oldNode = { ...newNode };
		let oldParent = tree.find((n) => n[propNames.nodePathProperty] == getParentNodePath(draggedPath));
		...
		let newParent =
			tree.find((x) => x[propNames.nodePathProperty] == getParentNodePath(newNode[propNames.nodePathProperty])) ?? null;
	...
	function handleDragEnter(e, node, el) {
		setTimeout(() => {
			...
			if (
				...
				(node[propNames.insertDisabledProperty] === true && node[propNames.nestDisabledProperty] === true)
			) {
				validTarget = false;
			}
			...
			if (dragEnterCallback) {
				//get node for event
				let draggedNode = tree.find((n) => n[propNames.nodePathProperty] == draggedPath);
				let oldParent = tree.find((n) => n[propNames.nodePathProperty] == getParentNodePath(draggedPath));
	...
	function highlightNesting(n, hn, vt, cn) {
		return cn && highlighThisNode(n, hn, vt) && n[propNames.nestDisabledProperty] !== true;
	}
	/**
	 * returns true, it should highlight nesting on this node
	 * @param n node
	...
	function highlightInsert(n, hn, vt, cn) {
		return !cn && highlighThisNode(n, hn, vt) && n[propNames.insertDisabledProperty] !== true;
	}
...
>
	{#each parentChildrenTree as node (getNodeId(node))}
		...
		>
			...
			<div
				class="tree-item
				{highlightNesting(node, highlightedNode, validTarget, canNest) ? expandClass : ''}
				...
				class:hover={highlightInsert(node, highlightedNode, validTarget, canNest) ||
				...
				{#if node[propNames.hasChildrenProperty]}
					...
					<span
						on:click={() =>
							toggleExpansion(
								node,
								shouldExpand(node[propNames.expandedProperty], childDepth, expandedLevel) &&
			...
			{#if shouldExpand(node[propNames.expandedProperty], childDepth, expandedLevel) && node[propNames.hasChildrenProperty]}
				<svelte:self
					...
					on:open-ctxmenu={(data) => {
						childDepth == 0 ? openContextMenu(data.detail.e, data.detail.node) : dispatch('open-ctxmenu', data.detail);
					}}
					{expandCallback}
					on:moved
					{beforeMovedCallback}
