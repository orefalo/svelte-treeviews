...
export function continuous<
  T extends (info: { count: number; currentCount: { value: number } }, ...args: any[]) => any
...
// typescript
export type ParametersWithoutFirst<T extends (...args: any) => any> = T extends (
  first: any,
  ...args: infer P
) => any
  ? P
 ...
 */
export type OmitFirstParameter<F> = F extends (x: any, ...args: infer P) => infer R
  ? (...args: P) => R
  : never
